...
import {ReturnsListRequest} from "../../APIRequest/ReturnAPIRequest";
...
const ReturnList = () => {
    ...
    useEffect(()=>{
        (async () => {
            await ReturnsListRequest(1,perPage,searchKeyword);
        })();
    },[])
    ...
    const handlePageClick = async (event) => {
        await ReturnsListRequest(event.selected + 1, perPage, searchKeyword)
    };
    ...
    const perPageOnChange=async (e) => {
        ...
        await ReturnsListRequest(1, e.target.value, searchKeyword)
    }
    const searchKeywordOnChange=async (e) => {
        setSearchKeyword(e.target.value)
        if ((e.target.value).length === 0) {
            setSearchKeyword("0")
            await ReturnsListRequest(1, perPage, "0")
        }
    }
