...
import {
    ...
    RemovePurchaseItem, ResetPurchaseFormValue,
    ResetPurchaseItemList,
    SetPurchaseItemList
...
const PurchaseCreateUpdate = () => {
    ...
    const CreateNewPurchase=async () => {
        ...
        if (IsEmpty(PurchaseFormValue.SupplierID)) {
            ...
            let Result = await CreatePurchaseRequest(PurchaseFormValue, PurchaseItemList);
            if(Result.status){
                                        ...
                                        <select onChange={(e)=>{store.dispatch(OnChangePurchaseInput({Name:"SupplierID",Value:e.target.value}))}} className="form-select form-select-sm">
                                            <option value="">Select Supplier</option>
                                            {
                                                SupplierDropDown.map((item,i)=>{
                                                    return( <option key={i} value={item._id}>{item?.SupplierName}</option>)
                                                })
                                            }
                                        </select>
                                    </div>
